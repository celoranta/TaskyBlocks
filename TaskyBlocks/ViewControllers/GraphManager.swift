...
//    if let siblings = node.parent?.tree{
//      return siblings
//    }
//    return [node]
//    }

  //Updates treepaths of all its parents' descendants before returning
//  func remove(node: TaskyNode) -> TaskyNode {
//    var index = 0
//    node.treePath = [0]
...
//  }
  ...
  fileprivate func createHierarchyGraph() {
    clearGraphs()
    createRootNodes()
    createDescendants(ofNodes: hierarchyGraph)
    createTree()
  }
  ...
  fileprivate func createDescendants(ofNodes nodeArray: [TaskyNode]) {
    for node in nodeArray {
      chartChildren(ofNode: node)
    }
  }
  
  //Creates a node for every incomplete task without a parent
  //And adds to graph collection
  fileprivate func createRootNodes(){
  ...
  fileprivate func calculateMaxGen(){
  ...
  func insert(node: TaskyNode, asChildTo parent: TaskyNode, at index: Int = 0) {
    parent.tree.insert(node, at: index)
    createTree()
  }
