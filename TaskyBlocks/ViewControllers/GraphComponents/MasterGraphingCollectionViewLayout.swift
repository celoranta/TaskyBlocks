...
public class MasterGraphingCollectionViewLayout: UICollectionViewLayout {
  ...
  override public var collectionViewContentSize: CGSize {
    return CGSize(width: contentWidth , height: contentHeight) // Here's where the height gets adjusted, but this is calculated from the graphing size
  }
  
  override public func prepare() {
    guard let collectionView = collectionView else { return }
    
    let numberOfColumns = Int(contentWidth / cellWidth) // #3  // The number of columns is calculated by rounding off the contentWidth/CellWidth
